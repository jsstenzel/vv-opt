...
def plot_nsga2(pareto_costs, pareto_utilities, design_pts, cost="hours", ylabel="expected posterior variance of "+r"$Q$", util_err=None, showPlot=False, savePlot=False, logPlotXY=[False,False]):
	costs_s, utilities = zip(*sorted(zip(pareto_costs, pareto_utilities)))
	
	if cost == "hours":
		costs = [c/3600 for c in costs_s]
		xlabel="total testbed time [hr]"
	elif cost == "M$":
		costs = [c/1000000 for c in costs_s]
		xlabel="total cost [M$]"
	else:
		costs = [c for c in costs_s]
		xlabel="total cost [$]"
