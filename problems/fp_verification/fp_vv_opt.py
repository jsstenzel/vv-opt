...
if __name__ == '__main__':  
	...
	def proposal_fn_norm(theta_curr):
		theta_prop = [0] * len(theta_curr)
		proposal_width = [.3**2,.75**2,.003**2]
		for i,_ in enumerate(theta_prop):
			#proposal dists are gammas, to match
			mean = abs(theta_curr[i])
			variance = proposal_width[i]
			theta_prop[i] = scipy.stats.norm.rvs(size=1, loc=mean, scale=np.sqrt(variance))[0]
		return theta_prop
	...
	if False: #play with mcmc_multivar
		...
		mcmc_multivar(y_nominal, likelihood_kernel, proposal_fn_gamma, fp.prior_rvs, fp.prior_pdf_unnorm, 10000, burnin=0, lag=1, doPlot=True, legend=fp.theta_names)
	
	if True: #play with mcmc_nolikelihood
		print("mcmc",flush=True)
		n_kde = 10**4
		n_mcmc = 10**2
