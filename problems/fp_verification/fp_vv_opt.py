...
def fp_vv_UP_QoI(problem, req, n=10000):
	#uncertainty propagation of HLVA
	uq_thetas = fp.prior_rvs(n)
...
def fp_vv_UP_exp(problem, dd,  n=10000, savefig=False):
	print("Likelihood distribution for nominal historical case:",flush=True)
	#tt = fp.prior_rvs(1); print(tt)
	tt = theta_nominal
	ysample_nominal = [fp.eta(tt, dd) for _ in range(n)]
...
def fp_vv_SA_exp(problem, dd, p=8):
...
def uncertainty_mc(problem, dd, n_mc=10**3, n_gmm=10**3, n_test=10**3):
	util_samples = []
	for ii in range(n_test):
		print(ii, flush=True)
		util, _ = U_varH_gbi(dd, problem, n_mc=n_mc, n_gmm=n_gmm, ncomp=5, doPrint=False)
...
if __name__ == '__main__':  
	...
	if args.run == "gbi_test":
		vv_gbi_test(problem, design, 10**6, y_nominal, ncomp=0)
	if args.run == "gbi_test_rand":
		vv_gbi_test(problem, design, 10**6, ncomp=0)
	...
	if args.run == "uncertainty_mc":
		util_samples = uncertainty_mc(problem, design)
		print(util_samples)
