...
def fp_likelihood_fn_err(theta, d, x):
	#define interest params:
	if type(theta) is dict:
		gain = theta["gain"]
		rn = theta["rn"]
	...
	err1 = gain_err(gain, rn, dc, t_gain, I_gain, x)
	err2 = read_noise_err(gain, rn, n_meas_rn, x)
	err3 = dark_current_err(gain, rn, dc, d_num, d_max, d_pow, x)
...
def gain_err(gain, rn, dc, t, I, _x):
	#define parameters:
	P_signal = _x["P_signal"] #probability of a signal event being correctly identified as an event; 1-P_signal is false negative rate
	P_noise = _x["P_noise"] #probability of noise being incorrectly identified as an event; P_noise is false positive rate
	sigma_dc = _x["sigma_dc"]
...
def read_noise_err(gain, rn, n_meas, _x):
	#define parameters:
	nx = _x["nx"]
	ny = _x["ny"]
	sigma_stray = _x["sigma_stray"] #e-
...
def dark_current_err(gain, rn, dc, d_num, d_max, d_pow, _x):
	#define parameters:
	mu_stray = _x["mu_stray"]
	sigma_stray = _x["sigma_stray"]
	sigma_dc = _x["sigma_dc"]
	...
	#Calculate error to each datapoint
	err_list = [0]*len(t_list)
	for i,_ in enumerate(err_list):
		ti = t_list[i]
		random_var = gain**2 * (rn**2 + (sigma_dc*ti)**2 + (sigma_stray*ti)**2) / np.sqrt(nx**2 * ny**2)
		random_sigma = math.sqrt(random_var)
		if random_sigma > 1e-12:
			err_list[i] = random_sigma
...
"""
Making joint distribution...
Generating quadrature...
Generating model evals...
Generating expansions...
6
6
6
Generating PCE model...
6
[array([8.20414453e+03, 4.15547121e+00, 1.08759935e-03]), array([1.24057334e+04, 9.39476821e+00, 6.09894834e-02]), array([-3.61633738e+05, -1.46934339e+02, -1.03289048e+00]), array([6.54299075e+05, 3.36844700e+02, 1.98134086e+00]), array([-7.16569541e+05,  9.04985788e+02,  3.06989138e+00]), array([ 5.17362669e+06, -1.62549025e+02, -1.38935313e+01])]
[ 4.77033237e+06  9.45897362e+02 -9.81311251e+00]
[6459.800119372048, 2.754677724320874, 0.001594588607672262]
