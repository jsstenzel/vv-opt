...
if __name__ == '__main__':  
	...
	d_worst = [
						...
						1, #d_max
						3,	 #d_pow   #approx, 
						1,  #n_qe #questionable; kind of wasn't measured
						.1,  #t_qe
						1, #d_vph_n_pts
						1, #d_dichroic_n_pts
						1 #d_frd_n_meas
	...
	y_nominal = problem.eta(theta_nominal, design, err=False)
	...
	if args.run == "UP_exp":
		vv_UP_exp(problem, design, theta_nominal, n=10)
	
	#Still needs massaging...
	#if args.run == "SA_exp":
	#	vv_SA_exp(problem, design)

	###Optimal Bayesian Experimental Design
	if args.run == "gbi_test":
		vv_gbi_test(problem, design, 10**2, y_nominal, ncomp=0)
	if args.run == "gbi_test_rand":
		vv_gbi_test(problem, design, 10**2, ncomp=0)
	
	if args.run == "obed_gbi":
		U_hist = vv_obed_gbi(problem, design)
	
	if args.run == "uncertainty_mc":
		util_samples = uncertainty_mc(problem, design, n_mc=10**2, n_gmm=10**2, n_test=3)
		print(util_samples)
