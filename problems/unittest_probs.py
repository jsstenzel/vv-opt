...
if False:
	...
	#kde_lines = np.array([np.linspace(left, right, n_kde) for left,right in theta_domains])
	#kde_thetas1, kde_thetas2, kde_thetas3 = np.meshgrid(kde_lines)
	#kde_thetas = np.zip(kde_thetas1, kde_thetas2, kde_thetas3)
	#print(kde_thetas)
	...
	likelihood_kernel, _ = general_likelihood_kernel(kde_thetas, bw_method=.1) #tuned by hand, just a little tail at the edges but that looks okay
	print(likelihood_kernel.covariance_factor())
	kde_plot(likelihood_kernel, kde_thetas, plotStyle='3d', ynames=['gain','rn','dc'])
	kde_plot(likelihood_kernel, kde_thetas, plotStyle='separate', ynames=['gain','rn','dc'])
