...
def nexus_lyapunov_system_model(theta, x, verbose=True):
	...
	jitter = eng.nexus_lyapunov(
		...
		matlab.double(theta["I_SMhubt"]),
		matlab.double(theta["I_SMhuba"]),
		matlab.double(x["m_RW"]),
		matlab.double(theta["K_yPM"]),
		matlab.double(theta["I_xRWA"]),
		matlab.double(theta["I_yRWA"]),
		matlab.double(theta["I_RWt"]),
		matlab.double(theta["I_RWa"]),
		matlab.double(x["m_ISO"]),
		matlab.double(theta["I_ISOa"]),
		matlab.double(theta["I_ISOt"]),
		matlab.double(theta["K_yISO"]),
		matlab.double(theta["K_xISO"]),
		matlab.double(x["m_RWAchx"]),
		matlab.double(theta["I_bus"]),
		matlab.double(x["m_bus"]),
		matlab.double(x["m_prop"]),
		matlab.double(theta["I_propt"]),
		matlab.double(theta["I_propa"]),
		matlab.double(x["m_instr"]),
		matlab.double(theta["I_i1"]),
		matlab.double(theta["I_i2"]),
		matlab.double(theta["I_i3"]),
		matlab.double(theta["A_sptop"]),
		matlab.double(theta["D_sp"]),
		matlab.double(theta["t_sp"]),
		matlab.double(theta["I_ss"]),
