function S=grwabroad_axi(f,Ro,dR,type,model,scale,haxi,caxi);
%rwabroad_axi   broadband PSD of reaction wheel  axial forces
%
%       S=grwabroad_axi(f,Ro,dR,type,model,scale,haxi,caxi)
%
%       where,  S	is the Power Spectral Density of the
%                	reaction wheel axial force disturbance (N^2/Hz)
%               Ro	is the nominal wheel speed in RPM (optional)
%               dR	is the expected variation about the nominal
%			(optional)
%               f	is the vector of frequencies (in Hz) for which
%                	S is calculated
%		type	type of distribution assumed for RWA speed
%               model   reaction wheel model chosen
%               scale   scale factor to change RW disturbance magnitude
%               haxi    harmonics axial
%               caxi    axial amplitude coefficients
%
...
%  08Aug98 odeweck:   added model variable for reaction wheel model
%  22Sep98 odeweck:   added scale variable
...
[Ci,hi]=grwadist_axi(1,model,scale,haxi,caxi);
